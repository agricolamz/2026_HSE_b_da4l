...
$$y_i \sim   Normal(\mu = \hat{\beta_0}, \sigma)$$
...
normal_fit <- brm(vowel.dur ~ 1, 
                  data = vowel_data,
                  family = gaussian(),
                  prior = c(prior(normal(275, 65), class = "Intercept", lb = 0),  # <1>
                            prior(normal(0, 1), class = "sigma", lb = 0)),        # <2>
                  silent = TRUE)
normal_fit
```

1. Аргументы `lb` и `ub` позволяют задать верхние и нижние границы априорного распределения.
2. Сообщая `brms` информацию `class = sigma` мы уже говорим, что это стандартное отклонение, и что оно всегда положительное, но я решил добавить для единообразия.
...
Так как случайный член --- стандартная часть регрессионных моделей, он показывает два одинаковых параметра `b_Intercept` и `Intercept`.
...
::: {.callout-note}
В работе [@stepanova11] сообщается, что средняя скорость речи на русском языке составляет 5.31 слогов в секунду (стандартное отклонение 1.93). Скачайте [данные](https://raw.githubusercontent.com/agricolamz/2026_HSE_b_da4l/master/data/speech_tempo_zvenigorod.csv) корпуса г. Звенигород (<https://lingconlab.ru/zvenigorod/>) и оцените скорость речи носителя `ea1976`, используя данные из [@stepanova11] в качестве априорного распределения. Визуализируйте получившееся распределение. Какой вывод можно сделать о скорости речи звенигородской носительницы?
:::
...
pp_check(fit)+
  geom_vline(xintercept = 5.31, linetype = 2, color = "tomato")+
  annotate(geom = "text", x =  7, y = 0.3, label = "[Stepanova 2011]")
```
