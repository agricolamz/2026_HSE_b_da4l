...
::: {.callout-note}
В работе [@ota2009] описывается эксперимент, в котором арабским и японским билингвам показывали английские слова и просили их определить, относятся ли они к одной семантической группе. В словах были в том числе и фонетически близкие слова, которые различаются фонемами, которых нет в L1. В [датасет](https://raw.githubusercontent.com/agricolamz/2026_HSE_b_da4l/master/data/ota2009_key_rock.csv) записан фрагмент данных, в которых стимулы не связаны. Постройте регрессионную модель, предсказывающую время реакции (`Words.RT`) на основе того, верно ли испытуемый (`Subject`) выполнил задание (`Words.ACC`). Я использовал sum contrast.
:::
...
df <- read_csv("https://raw.githubusercontent.com/agricolamz/2026_HSE_b_da4l/master/data/ota2009_key_rock.csv")

df$Words.ACC <- factor(df$Words.ACC)
contrasts(df$Words.ACC) <- c(1, -1)

fit_mixed <- brm(Words.RT ~ Words.ACC + (1|Subject),
                 data = df,
                 cores = n_cores, 
                 silent = TRUE, 
                 prior = c(prior(normal(1500, 500), class = "Intercept", lb = 0),
                           prior(normal(0, 500), class = "b"),
                           prior(normal(2000, 1000), class = "sigma", lb = 0),
                           prior(normal(2000, 1000), class = "sd")))
